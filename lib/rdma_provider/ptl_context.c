...
struct ptl_context *ptl_cnxt_create(void) {
  ...
  ret = PtlInit();
	...
	if (srv_nid) {
		ret = PtlNIInit((int)atoi(srv_nid), PTL_NI_MATCHING | PTL_NI_PHYSICAL, PTL_SERVER_PID, NULL, NULL,
				&cnxt->ni_handle);
	} else {
		SPDK_PTL_WARN("RDMACM: SERVER_NID not set. Using default nid PTL_IFACE_DEFAULT=0!");
		ret = PtlNIInit(PTL_IFACE_DEFAULT, PTL_NI_MATCHING | PTL_NI_PHYSICAL, PTL_SERVER_PID, NULL, NULL,
				&cnxt->ni_handle);
	}
...
struct ibv_pd *ptl_get_ibv_pd(struct ptl_context *cnxt) { return &cnxt->pd; }
